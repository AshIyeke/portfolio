...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.div
      ref={ref}
      // Set to fixed top-0 to ensure it stays at the top of the viewport
...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  useEffect(() => {
    const handleLocationChange = () => {
      ...
      if (index === -1) {
        index = items.findIndex((item) => item.link === currentHash);
      }
  ...
  return (
    <motion.div
      ...
      className={cn(
        "hidden flex-1 flex-row items-center justify-center space-x-4 text-sm font-medium text-zinc-600 transition duration-200 hover:text-zinc-800 lg:flex lg:space-x-4",
        className,
      )}
    >
      {items.map((item, idx) => (
...
export const NavbarButton = ({
  ...
  children: React.ReactNode;
  className?: string;
  variant?: "primary" | "secondary" | "dark" | "gradient" | "blur";
} & (
  ...
  const variantStyles = {
    ...
    blur: "bg-black/5 dark:bg-white/10 backdrop-blur-sm border border-black/10 dark:border-white/20 text-black dark:text-white hover:bg-black/10 dark:hover:bg-white/20",
  };
